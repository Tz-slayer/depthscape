...
鼠标视差需要高频通信（§6.3），而**导航视差**不需要：用户按一次键产生一个事件，
不是每帧一个。因此它在 V0.1 的「每次请求一个进程 / stdin+stdout」边界内就能工作，
不必等到通信升级。

驱动源是合成器导航本身，与鼠标无关：

```text
上下切换 workspace  →  场景垂直位移
左右切换 column     →  场景水平位移
```

手感定义（这三条决定了实现形态）：

**1. 位置是状态，不是事件。** 切到第 3 个 workspace 再切回来，画面必须回到原位。
所以偏移是**累计**出来的，而不是每次按键播一段动画——用一个绝对索引到位置的映射
需要在某个地方定义原点，而任何原点对「桌面不从第一个 workspace 开始」的用户都是错的。
累计让「当前位置」自然成为原点。

**2. 方向相反。** 往下切 workspace，场景往上走——像透过车窗看风景，人往前走，
景物向后掠过。`focus-column-right` 同理，场景向左。

**3. 两层反向移动。** 前景动 `s`，背景动 `-s × ratio`（默认 `ratio = 0.35`）。
只动前景、让背景静止的方案被否决了：一个浮动的剪影贴在不动的背景上，
读起来像渲染故障而不是纵深——前景从它被抠出来的景物上滑走，正是遮罩错位的样子。

```text
        navigated +1 workspace
                │
    ┌───────────┴───────────┐
    ▼                       ▼
background  −ratio·s     foreground  +s
    │                       │
    └─────── 合成 ──────────┘
              ▼
        相对位移 (1+ratio)·s
...
**壁纸没有平移余量。** 壁纸与输出宽高比相同时（5120×2880 vs 2560×1440，都是 1.7778），
`PreserveAspectCrop` 一个像素都不裁——平移 8px 就露边。所以：

- **背景层自带 spare area**，不依赖裁切余量。做法是让背景的容器比视口大，
  而 `Image` 仍按视口尺寸渲染并居中。**不能用 `scale` 放大**：
  `scale` 会绕中心重采样，静止时就使背景相对前景错位 `size·eps/2`——
  2% 的 overscan 在 1440p 上是 25px，比位移本身还大，静止画面会有重影。
- **前景层不需要余量**。它移开后露出的是下面的背景层，而背景层是不透明的，
  没有空隙要填——露出的正是"被前景挡住的东西"，这就是效果本身。

**偏移必须有上限。** 累计是无限的，切 20 次 workspace 画面会飘出去很远。
`maxTravel` 默认 6%，按输出的长边算，同时决定了背景需要多少 spare area。

#### 驱动信号的两个真实限制

订阅 niri 事件流实测得到的事实，两者都影响实现：

| 驱动 | 可用信号 | 限制 |
|---|---|---|
| workspace | `Workspace focused: <id>` + 查表得 `idx` | 信号只给 id，序号要查 |
| column | `Window focus changed: <id>` + 查 `pos_in_scrolling_layout[0]` | **没有独立的列变更事件** |

由此产生两条约束：

- **列号只能从焦点窗口反查**，且 niri 只在**该列存在窗口**时提供它。
  空 workspace 上没有焦点窗口，水平轴就停在那里不动——这是数据模型的固有约束，
  不是缺陷。
- **两个轴必须解耦。** 切 workspace 会顺带改变焦点窗口，而新 workspace 的窗口
  可能位于不同的列。若把这也算作水平导航，按上下键时画面会同时左右滑动，
  读起来像 bug。实现上用一个「上次观察到的列所属 workspace」来区分：
  列变化若伴随 workspace 变化，只重新确立基准，不累计。

`NiriService` 已经是 `pragma Singleton` 且被 DMS 自己的
`DesktopPluginWrapper.qml` 这样用（`CompositorService.isNiri && NiriService.inOverview`），
所以插件直接监听它即可，不需要自己起 `niri msg` 进程。

#### 一个容易静默失败的坑

`NiriService.focusedWorkspaceId` 声明为 `property string`，而每个 workspace 的
`id` 来自 `niri msg --json`，是**数字**。`2 === "2"` 在 JS 里是 `false`，
所以严格相等比较永远匹配不上，垂直轴会**静默失效**——没有报错，偏移永远是 0。
两处都要归一化成字符串再比。

#### 启动时的归零

恢复会话时 niri 会在 shell 启动过程中在 workspace 之间跳转，每一次看起来都和
用户导航一模一样。等第一帧上屏时，壁纸可能已经偏移了预算的四分之一。
从外部无法区分这两者，所以在启动 3 秒后把偏移清零一次；此后任何变化都是真实导航。
这个延时与 raise 兜底用同一个值——两者回答的是同一个问题（shell 是否已经安顿好），
分成两个数字只会多一处要同步的地方。

#### 实测（2026-09-20）

状态层（`qs ipc -i <id> call depthscape parallax`）：

| 操作 | foregroundY | 判定 |
|---|---|---|
| 基线 | 0 | — |
| `focus-workspace-down` | −28.8 | 一个 step（1440 × 2%）✓ |
| `focus-workspace-up` | 0 | 精确回到原点 ✓ |
| `focus-column-right` | foregroundX −38.4 | 一个 step（2560 × 1.5%），方向正确 ✓ |
| `focus-column-left` | 0 | 精确回到原点 ✓ |
| 静止重复读取 ×3 | 完全一致 | 无漂移 ✓ |

像素层：为把渲染单独测出来（真实导航会同时改变屏幕上的窗口，两者事后无法区分），
加了一个测量钩子直接设偏移。DP-1（空输出）上取四种组合，在两两之间比对
平均绝对差（0..255）：
...
| 偏移 0 两次 | **0.1155** | 对照组 |
| 偏移 90 两次 | **0.1750** | 对照组 |
| 偏移 0 vs 90（四对均值） | **3.363** | 处理组 |

对照均值 0.145 vs 处理均值 3.363，**比值 23.2 倍**。同状态重复测量几乎完全一致，
不同状态差异显著，且四种组合的对应关系严格成对——视差真实生效且可逆。
偏移 90 时四条边带的均值与最暗值同量级、顶部 30px 纯黑像素占比 0.000%，
**边缘没有露白**。
...
**导航视差（已实现，见 §3.4）。** 由 workspace / column 切换驱动，两层反向位移，
偏移是累计状态而非动画，带位移上限与启动归零。

- [x] 导航驱动位移（上下 workspace / 左右 column）
- [x] 前景背景差异化位移
- [x] 最大位移限制
- [x] 多输出下只移动被导航的那一块屏
- [ ] 鼠标视差（需要 §6.3 的通信升级）
- [ ] 按深度连续分层（目前只分前景/背景两层）
- [ ] 平滑动画（当前是瞬时跟随；导航本身没有中间帧，是否需要待定）
- [ ] Idle 时停止刷新
...
所以下表可复现，不依赖 profiler。复现脚本：`tools/bench.py`。
5120×2880 壁纸、模型已就绪、同一台机器取 5 次中位数（冷启动 2 次），
含进程启动与 JSON 解析：

| 场景 | 优化前 | 优化后 |
|---|---|---|
| 三级全命中（重复检查） | 112.2 ms | **3.0 ms** |
| 遮罩未命中（拖滑块） | 236.9 ms | **15.1 ms** |
| 精修 + 遮罩未命中 | 375.6 ms | 221.3 ms |
| 全部未命中（冷启动） | 1023.5 ms | 898.9 ms |
...
深度推理 ~672 ms、guided filter ~147 ms、解码 61 ms。
